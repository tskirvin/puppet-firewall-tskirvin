...
Puppet::Type.type(:firewall).provide :iptables, :parent => Puppet::Provider::Firewall do
  ...
  def _delete
    if @lines
      @lines.reverse.each do |line|
        iptables delete_args(line)
      end
    else
      iptables delete_args(properties[:line])
    end
  end
  ...
  def combine(rule)
    @lines = Array(properties[:line]) unless @lines
    @lines.push(rule.properties[:line])

    if rule.source
      if self.source.is_a?(Array)
        a = self.source.flatten
      else
        a = [self.source]
      end
      unless a.include?(rule.source)
        a.unshift(rule.source)
      end
      @property_hash[:source] = a.flatten
    end

    if rule.destination
      if self.destination.is_a?(Array)
        a = self.destination.flatten
      else
        a = [self.destination]
      end
      unless a.include?(rule.destination)
        a.unshift(rule.destination)
      end
      @property_hash[:destination] = a.flatten
    end
  end
  ...
  def self.rule_to_hash(line, table, counter)
    ...
    [:source, :destination].each do |prop|
      if ! hash[prop].nil?
        a = Array.new
        hash[prop].split(',').each do |e|
          a.unshift Puppet::Util::IPCidr.new(e).cidr
        end
        hash[prop] = a
      end
    end
  ...
  def delete_args(_line)
    count = []
    line = _line.gsub(/\-A/, '-D').split()
